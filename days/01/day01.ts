...
function solveFirst(params: LocationIdLists): number {
    ...
    const rightListSorted: number[] = params.rightList.toSorted((a, b) =>
        a - b
    );
...
function solveSecond(locationIdLists: LocationIdLists): number {
    ...
    for (const locationId of locationIdLists.leftList) {
        const occurrencesInRightList: number = locationIdLists.rightList.reduce(
            (total, element) => total += element === locationId ? 1 : 0,
            0,
        );
